...
class PlannerConfig:
    """
    Estructura de datos que centraliza todos los hiperparámetros (hyperparameters) que 
    gobiernan el comportamiento físico, matemático y algorítmico del robot.

    CONCEPTOS CLAVE (Parámetros vitales):
    - grid_size: Resolución interna del planificador. A veces la percepción escupe mapas de 200x200, 
      pero el planificador los re-escala a 240x240 para tener mayor fidelidad matemática al trazar curvas.
    - smooth_kernel: Nivel de "inflación" de obstáculos. Un núcleo gaussiano difumina los bordes 
      de los obstáculos para que el robot prefiera pasar lejos de las paredes en lugar de rozarlas.
    - footprint_px: El tamaño físico del chasis del robot proyectado en píxeles. Fundamental para el cálculo de colisiones.
    - best_k: Cuántas de las mejores trayectorias se promediarán para lograr un camino de conducción ultra-suave.
    - use_clustering: Activa la Inteligencia Artificial (K-Means) para decidir topológicamente si esquivar por 
      la izquierda o por la derecha antes de fusionar las curvas.
    """
    grid_size: int = 240
...
class PlannedPath:
    """
    Contenedor de resultados (Output Payload). Transporta absolutamente toda la información 
    de la decisión que acaba de tomar el planificador en este fotograma (frame).

    Propiedades:
    - visualization: Imagen renderizada para humanos (con todas las curvas y decisiones dibujadas).
    - cost_map: La matriz matemática pura de los costos de navegación.
    - known_mask: Máscara binaria que indica qué partes del mapa fueron realmente vistas por el sensor.
    - final_path_pixels: La trayectoria ganadora en coordenadas de matriz (filas, columnas).
    - final_path_xy_m: La trayectoria ganadora convertida a metros físicos reales (lista para enviar a los motores).
    - metadata: Tiempos, estados de error, uso de clústers y estadísticas empaquetadas en un diccionario.
    """
...
def traversability_to_cost(
    ...
    """
    Invierte matemáticamente la percepción del robot: convierte un mapa de "Transitabilidad" 
    en un mapa de "Costo" (Peligro).

    CONCEPTOS CLAVE:
    - Transitabilidad (Traversability): 1.0 significa "Asfalto perfecto/Libre", 0.0 significa "Muro/Obstáculo".
    - Costo (Cost/Risk): Los algoritmos de planificación buscan encontrar el camino con el número más pequeño posible. 
      Por lo tanto, el costo es el inverso de la transitabilidad: 1.0 significa "Choque seguro", 0.0 significa "Camino Libre".
      Fórmula base: `costo = 1.0 - transitabilidad`.
    - unknown_cost: A las zonas donde el sensor no ve (puntos ciegos o fuera del alcance de la cámara) se les asigna un 
      peligro por defecto moderado (ej. 0.2). Esto logra que el robot prefiera el asfalto que está seguro que existe, pero 
      se anime a explorar lo desconocido si no hay otra alternativa.
    """
...
def cost_to_vis(cost: np.ndarray, known_mask: np.ndarray | None = None) -> np.ndarray:
    """
    Traduce la matriz matemática de costos a una imagen a color (RGB) estandarizada para telemetría humana.
    - Canal ROJO: Intensidad del costo (Peligro = Rojo).
    - Canal VERDE: Transitabilidad libre (Seguridad = Verde).
    - Zonas desconocidas: Se pintan de negro (0, 0, 0) si no están presentes en la máscara `known_mask`.
    """
...
def _resize_float_map(cost_map: np.ndarray, grid_size: int) -> np.ndarray:
    """
    Re-escala el mapa de costos continuo al tamaño interno configurado para el planificador, 
    utilizando interpolación bilineal (Image.BILINEAR) para suavizar las transiciones de los valores flotantes.
    """
    return np.asarray(
...
def _resize_mask(mask: np.ndarray, grid_size: int) -> np.ndarray:
    """
    Re-escala máscaras binarias (booleanas / blanco y negro). Usa interpolación de 
    "Vecino más cercano" (Image.NEAREST) para evitar que se generen tonos grises irreales en los bordes de la máscara.
    """
...
def _resize_pixel(pixel_rc: tuple[int, int], src_shape: tuple[int, int], grid_size: int) -> tuple[int, int]:
    """
    Regla de tres simple en 2D: Convierte la coordenada puntual de un píxel de una matriz de tamaño original 
    a su posición geométrica equivalente en la nueva matriz redimensionada (grid_size).
    """
...
def _planner_path_to_bev_path(
    ...
    """
    Una vez que el planificador encuentra la ruta ideal en su tamaño de grilla interno (ej. 240x240), 
    esta función re-escala de manera vectorizada toda la trayectoria matemática de vuelta al tamaño 
    original de la matriz de observación física para poder convertirla correctamente a metros físicos reales.
    """
    h0, w0 = int(src_shape[0]), int(src_shape[1])
    n = int(grid_size)
...
def _draw_plan_overlay(
    ...
    """
    Capa de Depuración Visual (Debug Overlay). Dibuja todo el proceso cognitivo del robot encima del mapa.
    - Gris semi-transparente: El "banco candidato" masivo de trayectorias evaluadas.
    - Naranja semi-transparente: Las trayectorias que sobrevivieron a los filtros de colisión.
    - Blanco grueso: La ruta final escogida.
    - Punto Blanco (Abajo): Ubicación actual del chasis del vehículo.
    - Punto Cyan: Ubicación de la meta matemática a alcanzar.
    """
...
def _smooth_cost(cost_map: np.ndarray, smooth_kernel: int) -> np.ndarray:
    """
    Inflación de Obstáculos (Obstacle Inflation). 
    Aplica un filtro de desenfoque gaussiano (Gaussian Blur) sobre el mapa de costos.
    
    CONCEPTOS CLAVE:
    - Esto propaga suavemente el "peligro" hacia el exterior físico de las paredes y obstáculos. 
      Como resultado, el algoritmo de costo obligará matemáticamente al robot a mantener una 
      distancia de seguridad prudencial (un buffer), evitando que pase rozando los muros, lo cual 
      es vital dada la imprecisión de los motores en el mundo real.
    """
...
def _validate_config(config: PlannerConfig) -> None:
    """
    Validador estricto (Hard Safety Checks). 
    Evita fallos críticos de matemática divisoria por cero o corrupción de memoria abortando la 
    ejecución si los parámetros de configuración del robot carecen de sentido geométrico.
    """
...
def plan_on_bev(
    ...
    """
    Función Maestra del Planificador Local (Master Pipeline).

    CONCEPTOS CLAVE:
    Esta función ejecuta de principio a fin el ciclo cognitivo de conducción del vehículo:
    1. PREPARACIÓN: Mapea la meta en metros a coordenadas matriciales (píxeles), infla los obstáculos 
       (_smooth_cost) y redimensiona las matrices al formato estándar operativo interno del planificador.
    2. MUESTREO (Sampling): Inunda matemáticamente el espacio generando de forma predictiva cientos de 
       rutas polinomiales cuadráticas dirigidas en todas direcciones.
    3. FILTRADO DURO (Collision Checking): Evalúa el banco de caminos contrastándolo contra el volumen físico 
       del robot (footprint_px). Desecha inmediatamente las líneas que implican un choque.
    4. ANÁLISIS TOPOLÓGICO (Clustering): Si el usuario lo activó, aplica IA (K-Means adaptativo) para 
       descubrir si hay múltiples grupos viables (ej. rodear un poste por derecha vs por izquierda) y 
       fuerza una decisión hacia el grupo que mejor se alinee con la meta final global.
    5. EVALUACIÓN DE COSTOS (Costing): Promedia de manera ponderada (Best-K) las curvas óptimas sobrevivientes 
       para lograr un comando de volante progresivo, suave y orgánico.
    6. TRADUCCIÓN: Des-escala las trayectorias ganadoras devolviéndolas al plano métrico mundial real 
       para que el sistema de control del chasis las ejecute.

    Args:
        bev_traversability: Matriz donde 1.0 es libre y 0.0 es pared/obstáculo.
        observed_mask: Máscara que indica qué zonas vio realmente el sensor.
        goal_x_m: Coordenada lateral de la meta en metros (Positivo = Derecha).
        goal_y_m: Coordenada frontal de la meta en metros (Positivo = Adelante).
        bev_resolution_m: Escala real del mapa en metros por píxel.
        config: Objeto con los parámetros del comportamiento.
        candidate_path_bank: Caminos pre-calculados a inyectar (útil en simulaciones).

    Returns:
        PlannedPath: Objeto con todas las rutas, visualizaciones generadas y metadatos de decisión.
    """
    ...
    # Ruta de escape de seguridad: Si todas las trayectorias candidatas chocan.
    ...
    # Análisis topológico opcional usando Inteligencia Artificial (K-Means)
