...
class Bridge:
    ...
    def _girar_hacia(self, heading_rel_deg: float, step_deg: float = 20.0) -> None:
        """Gira en pasos chicos hacia heading_rel_deg (relativo al rumbo del
        robot al momento de llamar), re-verificando con el mapa despues de
        cada paso. Nunca se compromete de una sola vez a un angulo grande
        calculado de antemano, porque para el final del giro esos datos ya
        pueden estar viejos.
        """
        assert self.odometry is not None
        if abs(heading_rel_deg) < 1e-6:
            return
        ang = self.follower.angular_sign * math.copysign(self.follower.turn_speed, heading_rel_deg)
        ang = float(np.clip(ang, -self.follower.max_angular, self.follower.max_angular))
        paso_s = math.radians(step_deg) / max(self.follower.turn_speed, 1e-3)
