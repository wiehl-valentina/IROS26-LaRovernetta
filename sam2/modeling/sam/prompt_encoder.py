...
"""
=============================================================================
CODIFICADORES DE INDICACIONES (Prompt Encoders) - SAM 2
=============================================================================
Este módulo define cómo el modelo interpreta las instrucciones. Contiene el 
codificador original de SAM (que traduce clics, cajas y máscaras a matemáticas) 
y dos variantes personalizadas (Custom).

CONCEPTOS CLAVES:
- PromptEncoder Clásico: Convierte las interacciones del usuario en "Embeddings" 
  (vectores espaciales) que el modelo entiende.
- Custom Prompt Encoders (Variantes Autónomas): Son clases diseñadas para 
  "puentear" (ignorar) al usuario. En lugar de procesar clics reales, utilizan 
  tensores entrenables (`nn.Parameter`). Esto permite que el modelo "aprenda" 
  a generar sus propias indicaciones automáticamente durante el entrenamiento, 
  ideal para segmentación cero-clic (Zero-Shot) o tareas hápticas autónomas.
=============================================================================
"""
...
"""
---------------------------------------------------------------------------
1. PROMPT ENCODER ORIGINAL
---------------------------------------------------------------------------
Procesa tres tipos de entradas (prompts):
1. Puntos (Sparse): Coordenadas (X, Y) con una etiqueta (Positivo/Negativo).
2. Cajas (Sparse): Coordenadas extremas que delimitan un objeto.
3. Máscaras (Dense): Una imagen binaria completa indicando áreas previas.

Utiliza codificación posicional aleatoria (Fourier) para ubicar espacialmente 
los clics y convoluciones para reducir la resolución de las máscaras densas.
---------------------------------------------------------------------------
"""
...
"""
---------------------------------------------------------------------------
2. CUSTOM PROMPT ENCODER (Variante Autónoma Base)
---------------------------------------------------------------------------
Un reemplazo (drop-in) para el PromptEncoder original. Su particularidad es 
que ignora completamente las entradas del usuario (clics, cajas o máscaras).

CONCEPTOS CLAVES:
- Representaciones Aprendidas (Learned Embeddings): Declara `sparse_tokens` y 
  `dense_emb` como `nn.Parameter`. Esto significa que, mediante descenso de 
  gradiente, la red neuronal descubrirá por sí misma cuál es el "clic perfecto" 
  o la "máscara perfecta" para resolver la tarea genérica en la que se esté entrenando.
---------------------------------------------------------------------------
"""
...
"""
---------------------------------------------------------------------------
3. CUSTOM PROMPT ENCODER LARGER (Variante Autónoma Extendida)
---------------------------------------------------------------------------
Misma filosofía que el CustomPromptEncoder, pero añade mayor "Capacidad" al modelo.

CONCEPTOS CLAVES:
- Aumento de Parámetros: En lugar de usar los `nn.Parameter` directamente, 
  esta clase los pasa a través de una red neuronal multicapa (MLP para tokens 
  dispersos, Convoluciones para matrices densas). 
- Ventaja: Permite que el modelo extraiga y moldee de forma mucho más rica y no lineal 
  el "prompt automático" antes de inyectarlo al Mask Decoder. Útil en datasets más 
  complejos donde un token estático simple no es suficiente.
---------------------------------------------------------------------------
"""
